...
# Install script for the Rockpi SATA Hat
...
readonly APP_NAME="rockpi-penta"
...
# Copy config files
readonly CONFIG_FILE_PATH="/etc/${APP_NAME}.conf"
log "Copying config files as: ${CONFIG_FILE_PATH}"
cp "${SCRIPT_PATH}/${APP_NAME}.conf" "${CONFIG_FILE_PATH}"
chown "${INSTALL_TARGET_USER}:${INSTALL_TARGET_USER}" "${CONFIG_FILE_PATH}"
